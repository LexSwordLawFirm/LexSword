...
import { 
  ...
  MessageSquare, Send, Paperclip, Minimize2, Shield, Users, Award, FileText
} from 'lucide-react';

// --- FONT & THEME STYLES (Justica International Theme) ---
// This ensures the fonts load immediately without external CSS files
const GlobalStyles = () => (
  <style>{`
    ...
    .chat-widget { animation: slideIn 0.3s ease-out; }
...
// ==============================================================================
// 1. LIVE CHAT WIDGET (Global)
// ==============================================================================
const LiveChat = ({ userRole }) => {
  // Hide for Admin (Admin uses Dashboard) but show for public/clients
  ...
  const [sessionId] = useState(() => {
      ...
      if (!stored) {
          stored = 'visitor_' + Date.now().toString(36);
    ...
    <div className="fixed bottom-6 right-6 z-[9999] font-sans no-print">
      {!isOpen && (
        <button 
          ...
          className="bg-[#c5a059] text-white w-14 h-14 md:w-auto md:h-auto md:px-6 md:py-4 rounded-full shadow-2xl hover:scale-105 transition flex items-center gap-2 border-2 border-white"
        >
          <MessageSquare size={24} />
          <span className="hidden md:inline font-bold font-serif uppercase tracking-widest text-xs">Chat Now</span>
          <span className="absolute top-0 right-0 w-3 h-3 bg-green-500 rounded-full border border-white"></span>
        </button>
      )}

      {isOpen && (
        <div className="chat-widget bg-white w-[340px] h-[480px] rounded-lg shadow-2xl flex flex-col overflow-hidden border border-gray-200">
          {/* Header */}
          <div className="bg-[#111827] p-4 flex justify-between items-center text-white border-b-2 border-[#c5a059]">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gray-700 flex items-center justify-center border border-[#c5a059]">
                <Scale size={20} className="text-[#c5a059]"/>
              </div>
              <div>
                <h4 className="font-serif font-bold text-sm">LexSword Support</h4>
                <p className="text-[10px] text-[#c5a059] uppercase tracking-wider">Online</p>
              </div>
            </div>
          ...
          <div className="flex-1 bg-gray-50 p-4 overflow-y-auto space-y-3 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]">
            <div className="text-center"><span className="text-[10px] bg-gray-200 text-gray-500 px-2 py-1 rounded">Today</span></div>
            {/* Welcome Msg */}
            <div className="flex justify-start">
               <div className="max-w-[85%] bg-white border border-gray-200 p-3 rounded-tr-lg rounded-br-lg rounded-bl-lg text-sm text-gray-700 shadow-sm">
                  Welcome to <strong>LexSword Chambers</strong>. <br/>How can we assist you with your legal query today?
               </div>
            </div>

            {messages.map((m, i) => (
                <div key={i} className={`flex ${m.sender_role === 'client' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] p-3 text-sm shadow-sm ${m.sender_role === 'client' ? 'bg-[#111827] text-white rounded-tl-lg rounded-tr-lg rounded-bl-lg' : 'bg-white border border-gray-200 text-gray-800 rounded-tr-lg rounded-br-lg rounded-bl-lg'}`}>
                    {m.content}
                  </div>
                </div>
            ))}
          ...
          <div className="p-3 bg-white border-t flex gap-2 items-center">
            <input 
                className="flex-1 bg-gray-100 px-4 py-2 rounded-full text-sm outline-none focus:ring-1 focus:ring-[#c5a059]" 
                placeholder="Type your message..." 
                value={inputText} 
                onChange={e => setInputText(e.target.value)} 
                onKeyDown={e => e.key === 'Enter' && handleSend()}
            />
            <button onClick={handleSend} className="bg-[#c5a059] text-white p-2.5 rounded-full hover:bg-[#111827] transition"><Send size={16}/></button>
          </div>
        </div>
      )}
    </div>
...
// ==============================================================================
// 2. PUBLIC HOMEPAGE (JUSTICA THEME - REBUILT)
// ==============================================================================
...
const PublicHome = ({ onLoginClick, loading }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (response.ok) { setShowSuccessModal(true); e.target.reset(); }
    ...
    <div className="font-sans text-gray-800 bg-white">
      ...
      <div className="bg-[#111827] text-gray-400 text-xs py-3 px-6 md:px-12 hidden md:flex justify-between border-b border-gray-800">
         <div className="flex gap-6">
            <span className="flex items-center gap-2 hover:text-white transition"><Phone size={14} className="text-gold"/> +88 01911 008 518</span>
            <span className="flex items-center gap-2 hover:text-white transition"><Mail size={14} className="text-gold"/> bdkanoon@gmail.com</span>
         </div>
      ...
      {/* --- Navbar --- */}
      <nav className="sticky top-0 w-full bg-white z-50 shadow-md py-4 px-6 md:px-12 no-print">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          {/* Logo */}
          <div className="flex items-center gap-3">
            <div className="bg-[#111827] text-gold p-2 rounded-sm"><Scale size={28} strokeWidth={1.5} /></div>
            <div className="leading-tight">
              <h1 className="text-2xl font-serif font-bold text-[#111827] uppercase tracking-wide">LexSword</h1>
              <p className="text-[9px] text-gold font-bold tracking-[0.3em] uppercase">Chambers</p>
            </div>
          </div>

          {/* Desktop Menu */}
          <div className="hidden lg:flex items-center gap-8 text-xs font-bold text-[#111827] tracking-widest uppercase font-serif">
            <a href="#home" className="hover:text-gold transition">Home</a>
            <a href="#about" className="hover:text-gold transition">About</a>
            <a href="#practice" className="hover:text-gold transition">Practice Areas</a>
            <a href="#team" className="hover:text-gold transition">Attorneys</a>
            <a href="#contact" className="hover:text-gold transition">Contact</a>
          </div>

          {/* Actions */}
          <div className="flex items-center gap-4">
             <button onClick={onLoginClick} className="hidden md:flex items-center gap-2 border-2 border-[#111827] px-5 py-2 text-xs font-bold uppercase tracking-widest hover:bg-[#111827] hover:text-white transition">
        ...
        {/* Mobile Menu */}
        {menuOpen && (
          <div className="lg:hidden absolute top-full left-0 w-full bg-[#111827] text-white p-6 flex flex-col gap-4 font-serif text-center uppercase tracking-widest text-sm shadow-xl">
      ...
      {/* --- Hero Section --- */}
      <header id="home" className="relative h-[85vh] flex items-center bg-[#111827] overflow-hidden no-print">
         {/* Background Image Overlay */}
         <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1589829085413-56de8ae18c73?ixlib=rb-1.2.1&auto=format&fit=crop&w=1950&q=80')] bg-cover bg-center opacity-20"></div>
         ...
         <div className="max-w-7xl mx-auto px-6 relative z-10 w-full grid md:grid-cols-2 gap-12 items-center">
            <div className="space-y-8 text-white">
               <div className="flex items-center gap-4">
                  ...
                  <span className="text-gold font-bold tracking-[0.2em] uppercase text-xs">Supreme Court Lawyers</span>
               </div>
               ...
               <p className="text-gray-400 text-lg max-w-lg font-light leading-relaxed">
                  LexSword provides expert legal defense and strategic advocacy. We are committed to protecting your rights and securing your future with integrity.
               </p>
               <div className="flex gap-4 pt-4">
                  <a href="#contact" className="btn-justica">Free Consultation</a>
                  <a href="#practice" className="border border-white text-white px-8 py-4 font-bold tracking-widest text-xs uppercase rounded-sm hover:bg-white hover:text-black transition">Our Services</a>
               </div>
            </div>
         </div>
      </header>
      ...
      {/* --- Features Bar --- */}
      <div className="bg-gold py-12 relative z-20 -mt-16 mx-4 md:mx-12 max-w-7xl md:self-center md:mx-auto rounded-sm shadow-2xl grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-[#d4b373] no-print">
         {[
            {icon: Shield, title: "Expert Protection", text: "Defending rights with integrity."},
            {icon: Users, title: "Client Focused", text: "Tailored legal strategies."},
            {icon: Award, title: "Proven Results", text: "History of winning cases."}
         ].map((f, i) => (
            <div key={i} className="p-6 flex items-center justify-center gap-4 text-[#111827]">
               <div className="p-3 border-2 border-[#111827] rounded-full"><f.icon size={24}/></div>
               <div>
      ...
      <section id="about" className="py-24 bg-white no-print">
         <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-16 items-center">
            <div className="relative group">
               <div className="absolute top-4 left-4 w-full h-full border-4 border-gold rounded-sm transition-transform group-hover:translate-x-2 group-hover:translate-y-2"></div>
               <img src="/head.jpg" alt="Adv. Azadur Rahman" className="relative w-full h-[600px] object-cover shadow-2xl rounded-sm grayscale group-hover:grayscale-0 transition duration-700"/>
               <div className="absolute bottom-12 -right-6 bg-white p-8 shadow-2xl border-l-4 border-gold max-w-xs hidden md:block">
                  <p className="font-serif text-5xl font-bold text-[#111827]">15+</p>
                  <p className="text-gray-500 uppercase tracking-wider text-xs font-bold mt-2">Years of Experience</p>
               </div>
            </div>
            <div className="space-y-8">
               <div>
                  <h4 className="text-gold font-bold tracking-[0.2em] uppercase text-xs mb-2">About LexSword</h4>
                  <h2 className="text-4xl md:text-5xl font-serif font-bold text-[#111827] leading-tight">We Are The Most Trusted <br/> Law Firm.</h2>
               </div>
               <p className="text-gray-600 leading-relaxed text-justify">
                  LexSword is a premier law firm in Bangladesh, dedicated to providing top-tier legal services. Led by <strong>Advocate Azadur Rahman</strong>, we specialize in Civil, Criminal, and Writ jurisdictions. Our commitment to excellence and client satisfaction has made us a trusted name in the legal arena.
               </p>
               <div className="space-y-4 border-t border-gray-100 pt-6">
                  ...
                  <div className="flex items-center gap-4">
                     <div className="w-12 h-12 bg-gray-100 flex items-center justify-center rounded-full text-gold"><Award/></div>
                     <div><h5 className="font-serif font-bold text-lg">98% Success Rate</h5><p className="text-sm text-gray-500">Case winning history.</p></div>
                  </div>
               </div>
               <div className="pt-4">
                  <p className="font-serif font-bold text-xl text-[#111827] italic">"Justice Delayed is Justice Denied."</p>
                  <p className="text-sm text-gold font-bold mt-2 uppercase tracking-wider">- Adv. Azadur Rahman</p>
               </div>
            </div>
         </div>
      </section>
      ...
      <section id="practice" className="py-24 bg-[#f9f9f9] no-print">
         <div className="max-w-7xl mx-auto px-6 text-center mb-16">
            <h4 className="text-gold font-bold tracking-[0.2em] uppercase text-xs mb-3">Area of Practice</h4>
            <h2 className="text-4xl font-serif font-bold text-[#111827]">How We Can Help You</h2>
            <div className="w-16 h-[2px] bg-gold mx-auto mt-6"></div>
         </div>

         <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-3 gap-8">
            {[
               { icon: Gavel, title: "Criminal Defense", desc: "Expert defense in sessions and magistrate courts for bail and trial proceedings." },
               { icon: Scale, title: "Civil Litigation", desc: "Resolving complex property, land, and monetary disputes with expert advocacy." },
               { icon: Folder, title: "Writ Jurisdiction", desc: "Protecting fundamental rights through Writ Petitions in the High Court." },
               { icon: User, title: "Family Law", desc: "Compassionate legal support for divorce, child custody, and inheritance issues." },
               { icon: DollarSign, title: "Corporate Law", desc: "Company formation, banking disputes, and commercial contracts drafting." },
               { icon: FileText, title: "Legal Drafting", desc: "Professional deeds, wills, contracts and vetting of land documents." }
            ].map((p, i) => (
               <div key={i} className="bg-white p-10 shadow-lg hover:shadow-2xl transition duration-500 group border-b-4 border-transparent hover:border-gold">
                  <p.icon size={40} strokeWidth={1} className="text-gray-400 group-hover:text-gold transition duration-500 mb-6"/>
                  ...
                  <a href="#contact" className="text-xs font-bold uppercase tracking-widest text-[#111827] flex items-center gap-2 group-hover:gap-3 transition-all">Read More <ArrowRight size={14}/></a>
               </div>
            ))}
         </div>
      </section>
      ...
      <section id="team" className="py-24 bg-white no-print">
         <div className="max-w-7xl mx-auto px-6 text-center mb-16">
            <h4 className="text-gold font-bold tracking-[0.2em] uppercase text-xs mb-3">Expert Attorneys</h4>
            <h2 className="text-4xl font-serif font-bold text-[#111827]">Meet Our Team</h2>
         </div>
         <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-3 gap-8">
            <div className="group relative overflow-hidden bg-[#111827]">
               <img src="/head.jpg" className="w-full h-[500px] object-cover opacity-90 transition duration-700 group-hover:scale-110 group-hover:opacity-100"/>
               <div className="absolute bottom-0 left-0 w-full p-8 bg-gradient-to-t from-[#111827] to-transparent">
                  <h3 className="text-2xl font-serif font-bold text-white">Adv. Azadur Rahman</h3>
                  <p className="text-gold text-xs font-bold uppercase tracking-wider mt-1">Head of Chamber</p>
                  <div className="h-[2px] w-8 bg-gold mt-4 transition-all duration-500 group-hover:w-full"></div>
               </div>
            </div>
            {['Adv. Anisuru Rahman', 'Adv. Abdur Razzak'].map((name, i) => (
               <div key={i} className="group relative overflow-hidden bg-[#111827]">
                  <img src={`/team${i+1}.jpg`} className="w-full h-[500px] object-cover opacity-80 grayscale transition duration-700 group-hover:scale-110 group-hover:opacity-100 group-hover:grayscale-0"/>
                  <div className="absolute bottom-0 left-0 w-full p-8 bg-gradient-to-t from-[#111827] to-transparent">
                     <h3 className="text-2xl font-serif font-bold text-white">{name}</h3>
                     <p className="text-gold text-xs font-bold uppercase tracking-wider mt-1">Senior Associate</p>
                     <div className="h-[2px] w-8 bg-gold mt-4 transition-all duration-500 group-hover:w-full"></div>
                  </div>
               </div>
            ))}
      ...
      {/* --- Appointment / Contact (Dark) --- */}
      <section id="contact" className="bg-[#111827] text-white py-24 no-print relative overflow-hidden">
         <div className="absolute top-0 right-0 w-1/2 h-full bg-[#1a2336] skew-x-12 translate-x-20 hidden md:block"></div>
         <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-16 relative z-10">
            <div className="space-y-8">
               <h4 className="text-gold font-bold tracking-[0.2em] uppercase text-xs">Contact Us</h4>
               <h2 className="text-4xl md:text-5xl font-serif font-bold leading-tight">Need Legal Advice? <br/> Make an Appointment.</h2>
               <p className="text-gray-400 text-lg">We are available to assist you. Fill out the form or call us directly.</p>
               
               <div className="space-y-8 pt-4">
                  <div className="flex items-center gap-6">
                     <div className="w-16 h-16 border border-gray-700 rounded-full flex items-center justify-center text-gold"><Phone size={28}/></div>
                     <div><p className="text-xs text-gray-500 uppercase font-bold tracking-widest">Call Now</p><p className="text-2xl font-serif font-bold">+88 01911 008 518</p></div>
                  </div>
                  <div className="flex items-center gap-6">
                     <div className="w-16 h-16 border border-gray-700 rounded-full flex items-center justify-center text-gold"><Mail size={28}/></div>
                     <div><p className="text-xs text-gray-500 uppercase font-bold tracking-widest">Email</p><p className="text-2xl font-serif font-bold">bdkanoon@gmail.com</p></div>
                  </div>
               </div>
            </div>

            <div className="bg-white p-10 shadow-2xl">
               <h3 className="text-2xl font-serif font-bold text-[#111827] mb-8">Request A Quote</h3>
               <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                     <input name="name" placeholder="Full Name" className="w-full bg-[#f9f9f9] p-4 border border-transparent focus:border-gold outline-none text-gray-900 transition" required/>
                     <input name="phone" placeholder="Phone Number" className="w-full bg-[#f9f9f9] p-4 border border-transparent focus:border-gold outline-none text-gray-900 transition" required/>
                  </div>
                  <input name="email" placeholder="Email Address" className="w-full bg-[#f9f9f9] p-4 border border-transparent focus:border-gold outline-none text-gray-900 transition"/>
                  <select name="service" className="w-full bg-[#f9f9f9] p-4 border border-transparent focus:border-gold outline-none text-gray-900 transition">
                     <option>Civil Litigation</option><option>Criminal Defense</option><option>Writ Petition</option><option>Family Law</option><option>Commercial Law</option><option>Other</option>
                  </select>
                  <textarea name="message" rows="4" placeholder="Case Details" className="w-full bg-[#f9f9f9] p-4 border border-transparent focus:border-gold outline-none text-gray-900 transition"></textarea>
                  <button type="submit" className="w-full btn-justica">Submit Request</button>
               </form>
            </div>
         </div>
      </section>
      ...
      <footer className="bg-[#0b0f19] text-gray-500 py-16 border-t border-gray-900 no-print">
         <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-4 gap-12">
            <div className="space-y-6">
               ...
               <p className="text-sm leading-relaxed">Providing expert legal solutions with integrity and dedication since 2010. Your trusted partner in justice.</p>
            </div>
            <div>
         ...
         <div className="max-w-7xl mx-auto px-6 mt-12 pt-8 border-t border-gray-900 flex flex-col md:flex-row justify-between items-center text-xs">
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 no-print">
          <div className="bg-white p-10 text-center max-w-sm w-full animate-bounce-in shadow-2xl border-t-4 border-gold">
             ...
             <button onClick={() => setShowSuccessModal(false)} className="btn-justica w-full">CLOSE</button>
          </div>
        </div>
      )}
    </div>
...
// ==============================================================================
// 3. ADMIN DASHBOARD & APP CONTROLLER (Unchanged Logic)
// ==============================================================================
...
const ClientDashboard = ({ session, onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 font-sans">
      <nav className="bg-[#111827] text-white p-4 flex justify-between shadow-lg"><div className="flex gap-2 font-bold text-xl items-center"><Scale className="text-[#c5a059]"/> My Case Portal</div><button onClick={onLogout} className="text-red-400 font-bold flex gap-2 items-center text-sm"><LogOut size={16}/> Logout</button></nav>
      <main className="max-w-4xl mx-auto p-6"><h2 className="text-2xl font-bold mb-6 text-slate-800">My Cases</h2>
        {loading && <p>Loading...</p>}
        <div className="grid gap-6">{myCases.map(c => (<div key={c.id} className="bg-white p-6 rounded shadow border-t-4 border-[#c5a059]"><h3 className="text-2xl font-bold text-slate-900">{c.case_no}</h3><p>{c.party_name}</p><div className="bg-slate-100 p-4 mt-4 rounded"><p className="font-bold text-slate-600">Next Date: <span className="text-red-600">{c.next_date}</span></p><p className="text-lg font-bold text-[#c5a059]">{c.current_step}</p></div></div>))}</div>
      </main>
    </div>
  );
};
...
const AdminDashboard = ({ session, onLogout }) => {
  ...
  const fetchChatSessions = async () => {
      const { data } = await supabase.from('chat_messages').select('session_id, created_at').order('created_at', {ascending: false});
      if(data) {
  ...
  return (
    <div className="flex h-screen bg-slate-100 font-sans overflow-hidden text-slate-900">
      {/* Styles injected via GlobalStyles component at the top */}
      {mobileMenuOpen && <div className="fixed inset-0 bg-black/50 z-40 md:hidden no-print" onClick={() => setMobileMenuOpen(false)}></div>}

      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#111827] text-white flex flex-col shadow-2xl transition-transform duration-300 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static no-print`}>
        <div className="p-6 text-2xl font-bold font-serif text-[#c5a059] border-b border-gray-800 tracking-wider flex justify-between items-center">
          CHAMBERS <button className="md:hidden text-white" onClick={() => setMobileMenuOpen(false)}><X size={24}/></button>
        </div>
        <nav className="flex-1 p-4 space-y-2 mt-4">
          <button onClick={() => {setActiveTab('dashboard'); setMobileMenuOpen(false);}} className={`w-full flex items-center gap-3 p-3 rounded transition ${activeTab === 'dashboard' ? 'bg-[#c5a059] text-slate-900 font-bold' : 'text-gray-400 hover:bg-gray-800'}`}><Gavel size={20}/> Case Dashboard</button>
          <button onClick={() => {setActiveTab('messages'); setMobileMenuOpen(false);}} className={`w-full flex items-center gap-3 p-3 rounded transition ${activeTab === 'messages' ? 'bg-[#c5a059] text-slate-900 font-bold' : 'text-gray-400 hover:bg-gray-800'}`}><MessageCircle size={20}/> Messages</button>
          <button onClick={() => {setActiveTab('tasks'); setMobileMenuOpen(false);}} className={`w-full flex items-center gap-3 p-3 rounded transition ${activeTab === 'tasks' ? 'bg-[#c5a059] text-slate-900 font-bold' : 'text-gray-400 hover:bg-gray-800'}`}><ClipboardList size={20}/> Task Manager</button>
          <button onClick={() => {setActiveTab('calendar'); setMobileMenuOpen(false);}} className={`w-full flex items-center gap-3 p-3 rounded transition ${activeTab === 'calendar' ? 'bg-[#c5a059] text-slate-900 font-bold' : 'text-gray-400 hover:bg-gray-800'}`}><CalIcon size={20}/> Calendar</button>
          <button onClick={() => {setActiveTab('accounts'); setMobileMenuOpen(false);}} className={`w-full flex items-center gap-3 p-3 rounded transition ${activeTab === 'accounts' ? 'bg-[#c5a059] text-slate-900 font-bold' : 'text-gray-400 hover:bg-gray-800'}`}><DollarSign size={20}/> Accounts & Ledger</button>
        </nav>
        <button onClick={onLogout} className="m-4 p-3 flex items-center gap-2 text-red-400 hover:bg-gray-800 rounded"><LogOut size={20}/> Logout</button>
      </aside>

      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="md:hidden bg-[#111827] text-white p-4 flex justify-between items-center shadow-md shrink-0 no-print"><span className="font-bold font-serif text-[#c5a059]">LEXSWORD</span><button onClick={() => setMobileMenuOpen(true)}><Menu size={24}/></button></div>
            ...
            <div className="flex h-full bg-white rounded shadow-lg overflow-hidden border border-slate-200">
               <div className="w-1/3 border-r border-slate-200 bg-slate-50 flex flex-col">
                  ...
                  <div className="flex-1 overflow-y-auto">
                     {chatSessions.length === 0 && <p className="p-4 text-sm text-gray-500">No active chats.</p>}
                     {chatSessions.map((c, i) => (
                        <div key={i} onClick={() => loadChat(c.session_id)} className={`p-4 border-b cursor-pointer hover:bg-white transition ${activeChat === c.session_id ? 'bg-white border-l-4 border-l-[#c5a059]' : ''}`}>
                           <div className="flex items-center gap-3">
                              <div className="w-10 h-10 bg-slate-200 rounded-full flex items-center justify-center font-bold text-slate-600">V</div>
                              <div><p className="font-bold text-sm text-slate-900">Visitor</p><p className="text-xs text-slate-500">ID: {c.session_id.substring(0,8)}...</p></div>
                           </div>
                        </div>
                     ))}
                  </div>
                  ...
                  {activeChat ? (
                     <>
                        ...
                        <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-slate-50">
                           {chatMessages.map((m, i) => (<div key={i} className={`flex ${m.sender_role === 'admin' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[70%] p-3 rounded-lg text-sm shadow-sm ${m.sender_role === 'admin' ? 'bg-[#c5a059] text-white' : 'bg-white text-slate-800 border'}`}>{m.content}</div></div>))}
                           <div ref={messagesEndRef} />
                        </div>
                        <div className="p-4 bg-white border-t flex gap-2"><input className="flex-1 border p-2 rounded-full px-4 outline-none focus:border-[#c5a059]" placeholder="Reply..." value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendAdminMessage()}/><button onClick={sendAdminMessage} className="bg-[#111827] text-white p-2 rounded-full hover:bg-[#c5a059]"><Send size={20}/></button></div>
                     </>
          ...
          {activeTab === 'dashboard' && (
            <div className="no-print">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                ...
                <div className="flex gap-2 w-full md:w-auto"><button onClick={() => { setFormData({ court_type: 'Judge Court', case_nature: 'Civil Suit', status: 'Ongoing' }); setModalMode('addCase'); }} className="flex-1 items-center gap-2 bg-[#111827] text-white px-6 py-2 rounded shadow hover:bg-[#c5a059] font-bold flex justify-center"><Plus size={18}/> NEW CASE</button></div>
              </div>
              <div className="flex gap-0 mb-4 border-b-2 border-slate-300">
                  <button onClick={() => { setMainCaseTab('judge'); setCaseFilter('all'); }} className={`px-6 py-3 font-bold text-lg transition-all ${mainCaseTab === 'judge' ? 'border-b-4 border-[#c5a059] text-slate-900 bg-white' : 'text-gray-500 hover:text-slate-700'}`}>Judge Court ({getCounts('judge').all})</button>
                  <button onClick={() => { setMainCaseTab('high'); setCaseFilter('all'); }} className={`px-6 py-3 font-bold text-lg transition-all ${mainCaseTab === 'high' ? 'border-b-4 border-[#c5a059] text-slate-900 bg-white' : 'text-gray-500 hover:text-slate-700'}`}>High Court ({getCounts('high').all})</button>
              </div>
              <div className="mb-4 relative"><Search className="absolute left-3 top-2.5 text-gray-400" size={20}/><input type="text" placeholder="Search..." className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded focus:border-[#c5a059] outline-none text-slate-900" onChange={(e) => setSearchTerm(e.target.value)}/></div>
              <div className="flex flex-wrap gap-2 mb-6">
                {mainCaseTab === 'judge' ? (
                    ...
                    <>
                        <button onClick={() => setCaseFilter('all')} className={`px-3 py-1 rounded-full text-sm font-bold border ${caseFilter === 'all' ? 'bg-[#c5a059] text-slate-900' : 'bg-white'}`}>All</button>
      ...
      {modalMode === 'addCase' && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded shadow-2xl p-6 h-[80vh] overflow-y-auto">
             ...
             <button onClick={handleSaveCase} className="w-full bg-[#111827] text-white py-2 font-bold">SAVE</button>
             <button onClick={() => setModalMode(null)} className="w-full mt-2 text-slate-500">Cancel</button>
          </div>
        </div>
      )}
      {modalMode === 'updateStatus' && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"><div className="bg-white w-full max-w-md rounded p-6"><h3 className="font-bold mb-4">Update</h3><input type="date" className="w-full border p-2 mb-2" value={formData.next_date} onChange={e=>setFormData({...formData, next_date:e.target.value})}/><input className="w-full border p-2 mb-2" value={formData.current_step} onChange={e=>setFormData({...formData, current_step:e.target.value})}/><button onClick={handleUpdateStatus} className="w-full bg-[#111827] text-white py-2">Update</button><button onClick={()=>setModalMode(null)} className="w-full mt-2">Cancel</button></div></div>
      )}
